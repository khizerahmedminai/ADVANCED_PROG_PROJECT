...
const GuidList = (props) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        if(props.col === 'ApartmentGuid'){
          if(props?.values['BuildingGuid']){
            let filtered = items.filter(item => item['BuildingGuid'] === props?.values['BuildingGuid'])
            setItems(filtered)
          }
        }
      })
  ...
  }, [props?.col, props?.values?.BuildingGuid])
      ...
      return (
        <>
          <tr style={props?.isAps ? {backgroundColor: PropertiesColor(item?.['Number'])?.[0]} : null }>
            {info?.map((it, i) => {
              if (it['COLUMN_NAME'].includes('Guid') === false) {
                if (i === 0) {  
                  return (
                    <>
                      <th scope='row'>
                        <div className='form-check'>
                          <button
                            ...
                            onClick={(e) => {
                              if (props.isGrid) {
                                props?.setSelVal([...props.selVal, item['Guid']]);
                                props?.setView('none');
                              }
                              if (props.isAps) {
                                ...
                                props?.setData(props.col, item['Guid']);
                                item['Name']
